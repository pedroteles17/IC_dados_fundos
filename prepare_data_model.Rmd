...
```{r}
df_cota <- daily_data[['cota']] %>% 
  dplyr::filter(Data >= '2004-02-01')

df_cota_out <- as.data.frame(apply(df_cota[,-1], 2, function(x) length(x[(x <= -0.80 | x > 0.8) & !is.na(x)]))) %>% 
  set_names('outliers') %>% 
...
# Winsorizing
daily_data[['cota']][,-1][daily_data[['cota']][,-1] <= -0.8 | daily_data[['cota']][,-1] > 0.8] <- 0
```
...
get_regres_info <- function(ret_xts, name){
  ...
  ## Alpha
  alpha <- regres_coef[1, 1]
  ## Beta Market
  beta_mercado <- regres_coef[2, 1]
  ## Beta Size
  beta_size <- regres_coef[3, 1]
  ## Beta Value
  beta_value <- regres_coef[4, 1]
  ## Beta Mom
  beta_mom <- regres_coef[5, 1]
  ...
  stats <- data.frame(mir, cvar, track_error)
...
get_funds_stat <- function(start_pre_date, end_pre_date, funds_eleg) {
  ...
  funds_flow_pct <- as.data.frame(do.call("rbind", funds_flow_pct)) %>% 
    set_names("pct_flow") %>% 
    mutate(pct_flow = (funds_inflow$inflow - funds_outflow$outflow) / pct_flow)
