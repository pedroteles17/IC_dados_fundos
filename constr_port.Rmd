...
for (i in 1:n_meses) {
  ...
  # Definimos a proporcao de nao NAs de cada fundo
  prop_na <- apply(cota_periodo, 2, function(cota_fundo) sum(!is.na(cota_fundo), na.rm = TRUE) / length(cota_fundo))
  
  # Definimos uma variavel com o codigo dos fundos que possuem dados para mais de 80% dos dias
  fundos_eleg <- names(prop_na[prop_na >= 0.8])
