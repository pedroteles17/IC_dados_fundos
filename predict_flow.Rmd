---
title: "Machine Learning to predict flows"
...
cadastral_info$invest_qualificado <- ifelse(cadastral_info$invest_qualificado == 'Não', 0, 1)

cadastral_info$alavancado <- ifelse(cadastral_info$alavancado == 'Não', 0, 1)
...
cadastral_info$fundo_cotas <- ifelse(cadastral_info$fundo_cotas == 'Não', 0, 1)

cadastral_info$fundo_exclusivo <- ifelse(cadastral_info$fundo_exclusivo == 'Não', 0, 1)
...
* The fund needs to have at least R$50.000.000 of Assets Under Management to avoid the incubation bias;
* The fund can't be exclusive (only one investor).

```{r}
# 2019-02-19 ATE 2020-02-19
get_funds_eleg <- function(start_pre_date, end_pre_date, corte_pl){
  # First restriction
  funds_eleg <- cadastral_info %>% dplyr::filter(data_inicio <= start_pre_date & (data_fim >= end_pre_date | is.na(data_fim))) %>% 
    dplyr::filter(fundo_exclusivo != 0) %>%
    dplyr::select(codigo)
  ...
  # Third restriction
  funds_eleg <- daily_data[['patrim_liq']] %>% dplyr::filter(Data >= start_pre_date & Data <= end_pre_date) %>%
    dplyr::select(all_of(colnames(funds_eleg))) %>%
    dplyr::select_if(function(x) mean(x, na.rm = TRUE) >= corte_pl)
...
get_funds_stat <- function(start_pre_date, end_pre_date, funds_eleg){
  ...
  funds_aum <- lapply(funds_aum, function(x) mean(x[(length(x)-21):length(x)], na.rm = TRUE))
...
```{r}
get_dependent_var <- function(start_eval_date, end_eval_date, funds_eleg, flow_type){

  funds_flow <- daily_data[[flow_type]] %>% dplyr::filter(Data >= start_eval_date & Data <= end_eval_date) %>%
  dplyr::select(all_of(funds_eleg)) 
  
  funds_flow <- lapply(funds_flow, function(x) sum(x, na.rm = TRUE))
  
  funds_flow <- as.data.frame(do.call('rbind', funds_flow)) %>% set_names('flow')
...
```{r}
get_final_df <- function(start_pre_date, end_pre_date, start_crash_date, end_crash_date, corte_pl, flow_type){
  funds_eleg <- get_funds_eleg(start_pre_date, end_pre_date, corte_pl)
  ...
  funds_stat_crash <- get_dependent_var(start_crash_date, end_crash_date, funds_eleg, flow_type)
  
  funds_stat <- merge(funds_stat_crash, funds_ret_stat, by = 0) %>% 
    `rownames<-`(.$Row.names) %>%
...
n_months <- interval(start_eval_date %m-% days(1), as.Date('2021-12-31')) %/% months(12)
pred_flows <- vector('list', length = n_months)
for (i in 1:n_months) {
  
  start_eval_date <- as.Date(start_eval_date) %m+% months(12)
  ...
  pred_flows[[i]] <- get_final_df(start_estim_date, end_estim_date, start_eval_date, end_eval_date, 50000, 'capt_liq') %>%
    dplyr::select(!c(invest_qualificado, aplic_min_inic)) %>% 
    mutate_at(vars(forma_condominio, fundo_cotas, fundo_exclusivo),~ifelse(is.na(.x), 0, .x)) %>%
    mutate(year = year(end_estim_date), month = month(end_estim_date)) %>% 
    mutate(flow_pct = flow / aum, .before = 1)
}

pred_flows <- do.call('rbind', pred_flows)

```

```{r}
rm(cadastral_info, daily_data, ind_rf, diff_cadastral_daily)
```

```{r}
data <- r_to_py(pred_flows)
...
data = pd.concat((data,pd.get_dummies(data.month, dtype = 'float64')),1)

data = data.drop(['year', 'month', 'flow'], axis = 1)

```

```{python}
X, y = data.drop('flow_pct', axis = 1), data.flow_pct
...
print("MAE Baseline: ", mean_absolute_error(y_test, [base_pred] * len(y_test)))
print("RMSE Baseline: ", mean_squared_error(y_test, [base_pred] * len(y_test), squared=False))
...
```{python}
# setup the dataset
grid = setup(data=data, target='flow_pct', session_id=42, n_jobs=1)
